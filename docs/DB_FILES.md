...
## Recent SQL Additions

| Path | Purpose |
|---|---|
| `mcs/MCS_route_optimization.sql` | Adds MCS route optimization tables, common codes, and demo route graph data. Run after the base MCS schema is installed. |
| `mcs/MCS_route_demo_bypass_patch.sql` | Adds the missing demo bypass edge for NCM-01-01 to NCM-01-02 route tests. Run once if route optimization SQL was already applied. |
| `mcs/MCS_route_demo_cv01_block_bypass_patch.sql` | Adds a bypass path for the scenario where `E-NCM01-CV01` is blocked. |
| `mcs/MCS_add_failed_transfer_status.sql` | Adds `MCS_TF_STATUS = FAILED` for PLC error/interlock transfer failure scenarios. |
