...
| `eventStatus` | 권장 | 이벤트 상태 (의미상 필요, MCS 누락 검증 대상은 아님) | `NORMAL`, `ERROR`, `INTERLOCK` |
...
MCS는 모든 이벤트에서 `equipmentCd`, `eventType`를 필수로 검증합니다.
`targetType`이 `TRANSFER`인 경우 `targetId`도 필수입니다.
아래 표는 이벤트 타입별로 **추가로** 검증되는 필수 필드입니다.

> `eventStatus`는 이벤트 의미상 함께 보내야 하는 값이지만, MCS 누락 검증(`VALIDATION_FAILED`) 대상은 아닙니다.

| 이벤트 타입 | 추가 필수 필드 | 누락 시 영향 |
|---|---|---|
| `TRANSFER_STARTED` | `locationCd`, `toLocationCd`, `lotNo` | MCS가 현재 위치, 도착 위치, LOT를 확정할 수 없어 이동 시작 처리를 보류 |
| `TRANSFER_COMPLETED` | `locationCd`, `lotNo` | 완료 위치와 LOT 확인이 불가능해 재고 반영 보류 |
| `EQUIPMENT_RUNNING` | (추가 필수 없음 — 공통 필드만 검증) | 설비 가동 로그만 저장 |
| `EQUIPMENT_ERROR` | `errorCode`, `message` | 오류 원인 식별 불가, 운영자 조치 지연 |
| `ARRIVED_WRONG_LOCATION` | `errorCode`, `message` | 위치 불일치 원인·메시지 확인 불가 |
| `INTERLOCK_BLOCKED` | `errorCode`, `message` | 인터락 조건 확인 불가 |
